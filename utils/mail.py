...
def send_verification_mail(to, username, verification_code):
    ...
    logging.info(f"SEND MAIL: Sending verification email to {to}. Confirmation link: {confirmation_link}")
...
def send_password_reset_mail(to, username, password_reset_code):
    ...
    logging.info(f'SEND MAIL: Password reset email sent to {to} with link {reset_link}')
    print(f'SEND MAIL: Password reset email sent to {to} with link {reset_link}')


def send_new_device_login_mail(to, device_info):
    logging.info(f'SEND MAIL: Login from new device for user {to}: {device_info}')
